...
pipeline {
    ...
    parameters {
        choice(name: 'TAGS', choices: ['@WalletAPIs','@UserAPIs','@all'], description: 'Choisir le tag correspondant aux tests à exécuter')
    }
    ...
    stages {
        ...
        stage('Testing') {
            steps {
                ...
                sh "mvn clean test \"-Dkarate.options=--tags ${TAGS} classpath:karate\" "}
            }
        }
